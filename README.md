...
- **Session Tracking** — Track the item you're working on, auto-move through statuses, log progress from git
- **Slash Commands:**
  - `/rainyday:track <identifier>` — Start working on an item: load details, move to In Progress, persist tracking
  - `/rainyday:done` — Finish the tracked item: confirm and move to Review
...
The `track` / `done` pair gives you lightweight item awareness across a session:

```
# Start of session — pick up your item
/rainyday:track RD-42
```

Claude loads the item details, moves it to **In Progress**, and writes `.claude/current-task` in your working directory. This file survives context compression, so Claude stays aware of what you're working on even in long sessions.
...
Claude reads `.claude/current-task`, asks you to confirm, then moves the item to **Review** and clears the file.
...
**Track a bug fix across a session:**
```
/rainyday:track RD-42
# ... do the work ...
/rainyday:done
...
/rainyday:link-commit
```
Claude reads the commit, extracts `RD-42`, posts a comment with the commit hash, and offers to move the item to review.
